class RembgRedisService
  ...
  def get_result(generation_id)
    ...
    Rails.logger.error "Failed to parse rembg data for #{generation_id}: #{e.message}"
    nil
  end
  ...
  def get_status(generation_id)
    ...
    Rails.logger.error "Failed to parse rembg status for #{generation_id}: #{e.message}"
