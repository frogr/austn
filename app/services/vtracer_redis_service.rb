class VtracerRedisService
  ...
  def get_result(generation_id)
    ...
    Rails.logger.error "Failed to parse vtracer data for #{generation_id}: #{e.message}"
    nil
  end
  ...
  def get_status(generation_id)
    ...
    Rails.logger.error "Failed to parse vtracer status for #{generation_id}: #{e.message}"
  ...
  def result_key(generation_id)
    "vtracer:#{generation_id}"
  end

  def status_key(generation_id)
    "vtracer:#{generation_id}:status"
  end
end
